...
export function Pin({ pin: { postedBy, image, _id, destination, save } }) {
  const [postHovered, setPostHovered] = useState(false);
  const navigate = useNavigate();

  const user = fetchUser();

  const alreadSaved = !!save?.filter((item) => item.postedBy?._id === user.sub)
    ?.length;
  ...
  const deletePin = (id) => {
    client.delete(id).then.location.reload();
  };
